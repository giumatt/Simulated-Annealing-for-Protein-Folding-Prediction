...
type get_distance_element(int, int, int);

// Procedure Assembly
extern void normalize_avx(VECTOR);
extern void apply_rotation_avx(VECTOR, MATRIX);
...
MATRIX distances;	// Vettore delle distanze
...
type combined_energy(char* seq, int N) {
    ...
    for (int i = 0; i < N; i++) {
        ...
        for (int j = 0; j < N; j++) {
			if(hydrophobicity[seq[i] - 65] == -1.0f)
				break;
            if (i != j) {
                type dist = distances[get_distance_index(i, j, N)];
                ...
                if (dist < 10.0f) {
                    ...
                    if (j > i) {
                        ...
                        if (charge[seq[i] - 65] != 0.0f && charge[seq[j] - 65] != 0.0f && hydrophobicity[seq[j] - 65] != -1.0f) {
                            electrostatic_contribution += (charge[seq[i] - 65] * charge[seq[j] - 65]) / (dist * 4.0f);
                        }

                        // Contributo idrofobico
                        if (hydrophobicity[seq[j] - 65] != -1.0f) {
...
type get_distance_element(int i, int j, int N) {
    if (i > j) {
        // Scambia i e j per garantire i < j
        int temp = i;
        i = j;
	...
	return distances[i * (N - 1) - (i * (i + 1)) / 2 + (j - 1)];
}
